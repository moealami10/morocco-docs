...
export type NatureContrat = 'عمل مفتوح المدة' | 'عمل محدد المدة' | 'متدرب' | 'أخرى'
...
const STORAGE_KEY = 'kaghit:draft:attestation-de-travail-ar'
...
function validate(data: AttestationData): FormErrors {
  ...
  if (!data.nomEntreprise.trim())     err.nomEntreprise = 'هذا الحقل إلزامي'
  if (!data.adresseEntreprise.trim()) err.adresseEntreprise = 'هذا الحقل إلزامي'
  if (!data.nomSignataire.trim())     err.nomSignataire = 'هذا الحقل إلزامي'
  if (!data.qualiteSignataire.trim()) err.qualiteSignataire = 'هذا الحقل إلزامي'
  if (!data.nomEmploye.trim())        err.nomEmploye = 'هذا الحقل إلزامي'
  if (!data.cinEmploye.trim())        err.cinEmploye = 'هذا الحقل إلزامي'
  if (!data.posteOccupe.trim())       err.posteOccupe = 'هذا الحقل إلزامي'
  if (!data.natureContrat)            err.natureContrat = 'هذا الحقل إلزامي'
  if (data.inclureSalaire && !data.salaireBrut.trim()) {
    err.salaireBrut = 'هذا الحقل إلزامي عند تضمين الراتب'
  }
  if (!data.dateDebut)                err.dateDebut = 'هذا الحقل إلزامي'
  if (!data.toujoursEnPoste && !data.dateFin)
    err.dateFin = `هذا الحقل إلزامي إذا لم يكن الموظف بعد في الخدمة`
  if (!data.lieuEmission.trim())      err.lieuEmission = 'هذا الحقل إلزامي'
  if (!data.dateEmission)             err.dateEmission = 'هذا الحقل إلزامي'
  return err
}
...
const DocumentPreview: React.FC<PreviewProps> = ({ data }) => {
  ...
  const cnssLine = d.noCnss
    ? `, उत्पाद السعر الضمان الاجتماعي تحت رقم ${d.noCnss},`
    : ''

  const periodLine = d.toujoursEnPoste
    ? `بدءًا من ${formatDate(d.dateDebut)} ومُستمر في الخدمة إلى اليوم`
    : `من ${formatDate(d.dateDebut)} إلى ${formatDate(d.dateFin)}`
          ...
          <>
            ...
            {d.nomEntreprise && (
              <div className="mb-5">
                ...
                {d.iceEntreprise && (
                  <p className="text-neutral-500 text-[11px]">الرقم التعريفي للمقاولة: {d.iceEntreprise}</p>
                )}
              </div>
            )}
            ...
            <p className="mb-4 text-justify">
              ...
              {d.qualiteSignataire && (
                <>, نعمل بصفتنا ك<strong>{d.qualiteSignataire}</strong></>
              )}{' '}
              من الشركة{' '}
              <strong>{d.nomEntreprise || '___________'}</strong>
              {d.adresseEntreprise && (
                <>, ومقرها في {d.adresseEntreprise}</>
              )}
              , نؤكد بموجب هذه الشهادة أن&nbsp;:
            </p>
            ...
            <p className="mb-4 text-justify">
              ...
              في إطار عقد <strong>{d.natureContrat}</strong> ضمن شركتنا {periodLine}.
              {isSalaryIncluded && (
                <> وقد بلغ الراتب الشهري الصافي المعطى للمستفيد(ة) <strong>{d.salaireBrut} درهم</strong>.</>
              )}
            </p>

            {/* Closing formula */}
            <p className="mb-6 italic text-justify">
              تُسلم هذه الشهادة للمعني بالأمر لاستعمالها ما ترى أنه مناسب.
            </p>

            {/* Lieu / date */}
            <p className="text-right mb-1">
              حررت في {d.lieuEmission || '___________'} بتاريخ {formatDate(d.dateEmission)}
            </p>
            ...
            <div className="flex justify-end">
              <div className="border border-dashed border-neutral-300 rounded w-44 h-24 flex items-center justify-center">
                <span className="text-[10px] italic text-neutral-300">التوقيع و الختم</span>
              </div>
            </div>
          </>
        )}
...
const AttestationDeTravailPageAr: React.FC = () => {
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-10 sm:py-14">
      <Seo
        title="شهادة عمل المغرب — مولد مجاني عبر الإنترنت"
        description="نموذج شهادة عمل المغرب: املأ النموذج وقم بتنزيل شهادة العمل بصيغة PDF في بضع نقرات، 100% مجانًا."
        canonicalUrl="https://kaghit.com/ar/attestation-de-travail"
      />

      <PageHeading
        title="شهادة العمل"
        description="املأ النموذج أدناه. يتم إنشاء مستند PDF في متصفحك. معلوماتك تبقى في متصفحك ولا تُرسل إلى خوادمنا."
        icon={
        ...
        >
          ...
          <div className="mb-6 flex items-start gap-3 rounded-xl bg-blue-50 border border-blue-100 p-4 text-xs text-blue-900">
            ...
            <div className="leading-relaxed">
              <strong>ملاحظة توضيحية:</strong> يمكن طلب هذه الشهادة في أي وقت خلال العلاقة الوظيفية ولا يلزم قانونًا ذكر الراتب فيها. وهي تختلف عن <em>شهادة العمل</em> التي يلزم قانونًا على صاحب العمل تسليمها خلال 8 أيام من انتهاء العقد (المادة 72 من قانون الشغل المغربي).
            </div>
          </div>
            ...
            <div className="flex flex-col gap-5">
              <FormField id="nomEntreprise" label="اسم الشركة" required error={currentErrors.nomEntreprise}>
                <input
                  ...
                  placeholder="شركة مثال ذات مسؤولية محدودة"
                ...
                <p className="text-xs text-neutral-500 mt-1">
                  {data.nomEntreprise.length}/100 حرفًا
                </p>
              </FormField>
              ...
              <FormField id="adresseEntreprise" label="عنوان الشركة" required error={currentErrors.adresseEntreprise}>
                <input
                  ...
                  placeholder="123، boulevard Mohammed V، الدار البيضاء"
                ...
                />
                <p className="text-xs text-neutral-500 mt-1">
                  {data.adresseEntreprise.length}/100 حرفًا
                </p>
              </FormField>

              <FormField id="iceEntreprise" label="الرقم التعريفي للشركة" hint="اختياري — معرّف الشركة الموحد" error={currentErrors.iceEntreprise}>
                <input
                  id="iceEntreprise"
                  type="text"
                  maxLength={20}
                ...
                />
                <p className="text-xs text-neutral-500 mt-1">
                  {data.iceEntreprise.length}/20 حرفًا
                </p>
              </FormField>
            </div>
          </Card>
          ...
          <Card className="mb-6">
            ...
            <div className="flex flex-col gap-5">
              <FormField id="nomSignataire" label="الاسم الكامل للموقع" required error={currentErrors.nomSignataire}>
                <input
                  id="nomSignataire"
                  type="text"
                  maxLength={60}
                ...
                />
                <p className="text-xs text-neutral-500 mt-1">
                  {data.nomSignataire.length}/60 حرفًا
                </p>
              </FormField>

              <FormField id="qualiteSignataire" label="الصفة / المنصب" required error={currentErrors.qualiteSignataire}>
                <input
                ...
                />
                <p className="text-xs text-neutral-500 mt-1">
                  {data.qualiteSignataire.length}/80 حرفًا
                </p>
              </FormField>
            </div>
          </Card>
          ...
          <Card className="mb-6">
            ...
            <div className="flex flex-col gap-5">
              <FormField id="nomEmploye" label="الاسم الكامل للموظف(ة)" required error={currentErrors.nomEmploye}>
                <input
                  id="nomEmploye"
                  type="text"
                  maxLength={60}
                  ...
                  placeholder="فاطمة الزهراء بن علي"
                ...
                />
                <p className="text-xs text-neutral-500 mt-1">
                  {data.nomEmploye.length}/60 حرفًا
                </p>
              </FormField>

              <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
                <FormField id="cinEmploye" label="بطاقة التعريف الوطنية للموظف(ة)" required error={currentErrors.cinEmploye}>
                  <input
                    id="cinEmploye"
                    type="text"
                    maxLength={20}
                  ...
                  />
                  <p className="text-xs text-neutral-500 mt-1">
                    {data.cinEmploye.length}/20 حرفًا
                  </p>
                </FormField>
                ...
                <FormField id="noCnss" label="رقم الضمان الاجتماعي" hint="اختياري" error={currentErrors.noCnss}>
                  ...
                  />
                  <p className="text-xs text-neutral-500 mt-1">
                    {data.noCnss.length}/20 حرفًا
                  </p>
                </FormField>
              </div>

              <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
                <FormField id="posteOccupe" label="المنصب المحتل" required error={currentErrors.posteOccupe}>
                  <input
                    id="posteOccupe"
                    type="text"
                    maxLength={80}
                  ...
                  />
                  <p className="text-xs text-neutral-500 mt-1">
                    {data.posteOccupe.length}/80 حرفًا
                  </p>
                </FormField>
                ...
                <FormField id="natureContrat" label="نوع العقد" required error={currentErrors.natureContrat}>
                  <div className="relative">
                    ...
                    >
                      <option value="عمل مفتوح المدة">عمل مفتوح المدة (CDI)</option>
                      <option value="عمل محدد المدة">عمل محدد المدة (CDD)</option>
              ...
              <div className="flex flex-col gap-3 pt-2 border-t border-neutral-100">
                <label htmlFor="inclureSalaire" className="flex items-center gap-3 cursor-pointer select-none group">
                  ...
                  <span className="text-sm font-medium text-neutral-700">
                    تضمين الراتب <span className="text-xs font-normal text-neutral-500">(شهادة عمل وراتب)</span>
                  </span>
                </label>

                {data.inclureSalaire && (
                  <div className="mt-1 pl-12">
                    <FormField id="salaireBrut" label="الراتب الشهري الصافي (درهم)" required error={currentErrors.salaireBrut}>
                      <input
                        id="salaireBrut"
                        type="text"
                        maxLength={20}
                      ...
                      />
                      <p className="text-xs text-neutral-500 mt-1">
                        {data.salaireBrut.length}/20 حرفًا
                      </p>
                    </FormField>
                  </div>
                )}
              ...
              <FormField id="dateDebut" label="تاريخ البدء" required error={currentErrors.dateDebut}>
                <input
                  id="dateDebut"
                  type="date"
              ...
              {/* لا يزال في الخدمة التبديل الخدمة */}
              <div className="flex items-center gap-3 py-1">
                ...
                >
                  ...
                  <span className="text-sm font-medium text-neutral-700">
                    ما زال في الخدمة الآن
                  </span>
                </label>
              </div>

              {/* تاريخ الانتهاء الشرطي */}
              {!data.toujoursEnPoste && (
                <FormField id="dateFin" label="تاريخ الانتهاء" required error={currentErrors.dateFin}>
                  <input
                    id="dateFin"
                    type="date"
                    value={data.dateFin}
          ...
          <div className="mt-5 flex items-start gap-2.5 rounded-lg bg-amber-50 border border-amber-100 px-4 py-3">
            ...
            <p className="text-xs text-amber-700 leading-relaxed">
              <strong>مهم:</strong> يجب طباعة هذا المستند وتوقيعه وختامه من قبل صاحب العمل ليكون ساريًا. ولا يُشكل هذا المستند نصيحة قانونية أو مالية.
            </p>
          </div>
        </form>
      ...
      {/* ── معلومات / قسم تحسين محركات البحث ── */}
      <section className="mt-16 border-t border-neutral-200 pt-12">
        <h2 className="text-xl font-bold text-neutral-900 mb-6">
          كل ما تحتاج معرفته حول شهادة العمل في المغرب
        </h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <Card>
            <h3 className="text-sm font-semibold text-neutral-900 mb-2 flex items-center gap-2">
              ...
              الدور والأهمية في المستند
            </h3>
            <p className="text-xs text-neutral-600 leading-relaxed">
              شهادة العمل هي وثيقة رسمية تُصدر عن صاحب العمل تؤكد أن الموظف يعمل أو عمل سابقاً في الشركة. تُثبت وجود عقد العمل، والأقدمية، والوظيفة المُشغلة.
            </p>
          </Card>
          ...
          <Card>
            <h3 className="text-sm font-semibold text-neutral-900 mb-2 flex items-center gap-2">
              <span className="w-2 h-2 rounded-full bg-primary" />
              الاستخدام المتكرر في المغرب
            </h3>
            <p className="text-xs text-neutral-600 leading-relaxed">
              في المغرب، يُطلب غالبًا شهادة العمل لفتح حساب بنكي، وطلبات القروض، ومستندات التأشيرة، أو اشتراكات التأمين، أو للإجراءات الإدارية لدى الصندوق الوطني للضمان الاجتماعي والصندوق المشترك.
            </p>
          </Card>
          ...
          <Card>
            <h3 className="text-sm font-semibold text-neutral-900 mb-2 flex items-center gap-2">
              <span className="w-2 h-2 rounded-full bg-primary" />
              القيمة القانونية و التوقيع
            </h3>
            <p className="text-xs text-neutral-600 leading-relaxed">
              يتيح هذا النموذج الإلكتروني إنشاء مستند conforme للusage الشائع. وللحصول على قيمة قضائية، يجب طباعة الشهادة الأصلية وتوقيعها من قبل الممثل القانوني للشركة ووضع ختم الشركة الرسمي. لا يُستبدل هذا النموذج بالنصيحة القانونية.
            </p>
          </Card>
        </div>
      </section>
