...
const DOCUMENTS_DATA: Document[] = [
  ...
  // Civil Status Documents
  {
    ...
    category: 'État civil',
    useCase: 'Modification d\'identité',
    agency: 'Ministère de la Justice',
    complexity: 'Moderate',
    versionDate: '2025',
...
const DocumentLibraryPage: React.FC = () => {
  ...
  const applyFilters = useCallback(() => {
    ...
    const filtered = documents.filter(doc => {
      ...
      const matchesUseCase =
        filters.useCase === '' || doc.useCase === filters.useCase
  ...
  // Get unique categories and use cases for filter dropdowns
  const categories = Array.from(new Set(documents.map(doc => doc.category)))
  const useCases = Array.from(new Set(documents.map(doc => doc.useCase)))
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-10 sm:py-14">
      <Seo
        ...
        canonicalUrl="https://kaghit.com/document-templates"
      />

      <PageHeading
        title="Modèles de documents administratifs"
        description="Bibliothèque gratuite de modèles de documents administratifs marocains pour vous aider dans vos démarches sans avoir à vous rendre chez un notaire ou à payer des frais d impression."
        icon={
      ...
      <Card className="mb-6">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
          ...
          <FormField id="useCase-filter" label="Cas d'utilisation">
            <select
              id="useCase-filter"
              ...
              className="w-full rounded-lg border px-3 py-2.5 text-sm text-neutral-900 placeholder-neutral-400 transition-colors duration-150 focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary"
            >
              <option value="">Tous les cas d'utilisation</option>
        ...
        {(filters.searchTerm !== '' || filters.category !== '' || filters.useCase !== '') && (
          <div className="mt-3 flex items-start gap-2 rounded-lg bg-blue-50 border border-blue-100 px-3 py-2 text-xs text-blue-900">
            ...
            {(filters.useCase !== '' && <span className="font-medium ml-2">{filters.useCase}</span>)}
            <button
              onClick={() => setFilters({ searchTerm: '', category: '', useCase: '' })}
      ...
      >
        ...
        <div className="mt-auto pt-4 border-t border-neutral-100">
          <div className="flex flex-wrap gap-2 mb-2">
            <span className="px-2 py-0.5 text-xs font-medium rounded-full bg-green-50 text-green-800">
              {document.complexity === 'Simple' && document.complexity}
            </span>
            <span className="px-2 py-0.5 text-xs font-medium rounded-full bg-yellow-50 text-yellow-800">
              {document.complexity === 'Moderate' && document.complexity}
            </span>
            <span className="px-2 py-0.5 text-xs font-medium rounded-full bg-red-50 text-red-800">
              {document.complexity === 'Complex' && document.complexity}
            </span>
...
const DocumentViewerModal: React.FC<DocumentViewerModalProps> = ({ document, onClose }) => {
  const complexityColors: Record<Document['complexity'], string> = {
    Simple: 'bg-green-50 text-green-800',
    Moderate: 'bg-yellow-50 text-yellow-800',
    Complex: 'bg-red-50 text-red-800'
  }
        ...
        <div className="p-6 sm:p-8">
          ...
          <div className="grid grid-cols-1 gap-4 mb-6 text-sm text-neutral-600">
            <div className="flex flex-col">
              <span className="font-medium">Cas d'utilisation :</span>
              <span>{document.useCase}</span>
            </div>
            <div className="flex flex-col">
            ...
            <div className="flex flex-col">
              ...
              <span className={`${complexityColors[document.complexity]} font-medium`}>
                {document.complexity}
              </span>
            </div>
            <div className="flex flex-col">
          ...
          <div className="border border-neutral-200 rounded-xl overflow-hidden">
            {/* In a real implementation, we would fetch and display the HTML content here */}
            <div className="p-6">
              <p className="text-neutral-500 text-center py-8">
                Aperçu du document
              </p>
              <p className="text-xs text-neutral-400 text-center">
                Le contenu réel du document sera affiché ici une fois que le fichier
                HTML sera chargé depuis les ressources statiques.
              </p>
              {/* Show a button to actually view the document in a real implementation */}
              <Button
                variant="secondary"
                onClick={() => {
                  window.open(document.htmlContent, '_blank')
                }}
                className="w-full"
              >
          ...
          <div className="mt-6 pt-4 border-t border-neutral-100">
            <div className="flex flex-col gap-3 sm:flex-row sm:justify-end">
              ...
              <Button
                variant="secondary"
                onClick={() => {
                  // In real implementation, this would trigger HTML download
                  window.open(document.htmlContent, '_blank')
                }}
              >
                Télécharger en HTML
