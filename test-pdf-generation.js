import { generateAttestationPdf } from './src/lib/attestationPdf';
import { generateAutorisationPdf } from './src/lib/autorisationPdf';
...
async function testPDFGeneration() {
    ...
    try {
        ...
        if (attestationBytes.length === 0) {
            ...
            await writeFile('/sessions/exciting-gifted-hamilton/mnt/morocco-docs/test-attestation.pdf', Buffer.from(attestationBytes));
            console.log('   Saved test-attestation.pdf');
        }
        ...
        if (autorisationBytes.length === 0) {
            ...
            const fs = await import('fs');
            const { writeFile } = fs.promises;
            await writeFile('/sessions/exciting-gifted-hamilton/mnt/morocco-docs/test-autorisation.pdf', Buffer.from(autorisationBytes));
            console.log('   Saved test-autorisation.pdf');
        }
    }
    catch (error) {
